...
namespace LightController.MyForm.Multiplex
{
	public partial class OldDetailMultiForm : Form
	{
		...
		public OldDetailMultiForm(MainFormBase mainForm, int lightIndex, IList<int> tdIndices, IList<StepWrapper> stepWrapperList)
		{
			this.mainForm = mainForm;

			InitializeComponent();
			Text = "单通道多步联调(" + mainForm.LightAstList[lightIndex].LightType + ":" + mainForm.LightAstList[lightIndex].LightAddr + ")";
		...
		public OldDetailMultiForm(MainFormBase mainForm, Dictionary<int, List<int>> tdDict) {
		...
		/// <summary>
		/// 辅助方法：测试这个tdIndex是否属于多灯之一
		/// </summary>
		private bool checkIfMultiLight(int lightIndex) {
