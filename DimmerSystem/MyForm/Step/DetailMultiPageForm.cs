...
{
    public partial class DetailMultiPageForm : UIForm
    {
        private MainFormBase mainForm;
        private int maxStep = 0;  // 所有选中灯具的最大步数
        private SortedSet<int> stepSet = new SortedSet<int>();  // 记录选中的步数（因为有分页，此项必需；另外通道选择因为都在页面内，无需处理）
        ...
        private void addTdPanel(IList<StepWrapper> stepWrapperList, int lightIndex, int tdIndex)
        {
            ...
            Panel tdSmallPanel = new Panel
            {
                ...
                Padding = tdSmallPanelDemo.Padding,
                Tag = lightIndex
            };
            ...
            {
                UIComboBox saComboBox = new UIComboBox
                {
                    ...
                    Tag = lightIndex,
                    BackColor = saComboBoxDemo.BackColor,
                    FillColor = saComboBoxDemo.FillColor,
                    FillDisableColor = saComboBoxDemo.FillDisableColor,
                    Font = saComboBoxDemo.Font,
                    ForeColor = saComboBoxDemo.ForeColor,
                    ForeDisableColor = saComboBoxDemo.ForeDisableColor,
                    Margin = saComboBoxDemo.Margin,
                    MinimumSize = saComboBoxDemo.MinimumSize,
                    Padding = saComboBoxDemo.Padding,
                    Radius = saComboBoxDemo.Radius,
                    RectColor = saComboBoxDemo.RectColor,
                    RectDisableColor = saComboBoxDemo.RectDisableColor,
                    Style = saComboBoxDemo.Style,
                    TextAlignment = saComboBoxDemo.TextAlignment,
                };
        ...
        private void groupComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (groupComboBox.SelectedIndex > 0)
            {
                int firstLightIndex = mainForm.GroupList[groupComboBox.SelectedIndex - 1].LightIndexList[0];
        ...
        {
            //CheckBox cb = sender as CheckBox;
            //(cb.Parent as Panel).BackColor = cb.Checked ? Color.DarkSlateGray : tdCheckBoxDemo.BackColor;
        }
        ...
        private void stepNUD_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            // 当下拉框尚未选中时，return；
            UIComboBox cb = (nud.Parent.Parent.Parent.Controls[0].Controls[1] as UIComboBox);
                ...
                for (int tdPanelIndex = 1; tdPanelIndex < bigFLP.Controls.Count; tdPanelIndex++)
                {
                    ...
                    if ((tdPanel.Controls[0].Controls[0] as CheckBox).Checked)
                    {
                        foreach (int curStepIndex in stepSet)
                        {
                            ...
                            mainForm.SetTdStepValue(curLightIndex, curTdIndex, curStepIndex, stepValue, false); //这些被动设置的通道值，就不要跳转了
                        }
                    }
                }
            }

            isValueChangedWork = false;
            refreshPage();
            isValueChangedWork = true;
        }
