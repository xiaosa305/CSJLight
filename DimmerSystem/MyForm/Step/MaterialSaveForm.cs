...
{
    public partial class MaterialSaveForm : UIForm
    {
		...
		public MaterialSaveForm(MainFormBase mainForm, IList<StepWrapper> stepWrapperList, LightAst la, int currentStep)
		{	
			this.mainForm = mainForm;
			this.stepWrapperList = stepWrapperList;
			stepCount = stepWrapperList.Count;
			StepWrapper firstStep = stepWrapperList[0]; //如果stepWrapperList.Count == 0 ,无法进入此Form 
			...
			modeLabel.Text += mainForm.CurrentMode == 0 ? "常规模式" : "音频模式";
			lightLabel.Text += lightName + " - " + lightType;

			materialPath = IniHelper.GetSavePath() + @"\LightMaterial\";
			materialPath += mainForm.CurrentMode == 0 ? "Normal" : "Sound";
		...
		private void saveButton_Click(object sender, EventArgs e)
		{
			...
			else
			{
				...
				// 2.将相关文件拷贝到文件夹内	
				string sourcePath = Application.StartupPath + @"\Default\materialSet.ini";
