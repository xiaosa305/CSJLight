    ...
    public class LightControllerSCR
    {
        ...
        public static LightControllerSCR Build(byte[] data)
        {
            try
            {
                ...
                {
                    light.ScrData[sceneIndex,0] = data[point + sceneIndex * 4 + 0] == Convert.ToByte(0xFF) ? 1 : data[point + sceneIndex * 4 + 0];
                    light.ScrData[sceneIndex,1] = data[point + sceneIndex * 4 + 1] == Convert.ToByte(0xFF) ? 1 : data[point + sceneIndex * 4 + 1];
                    light.ScrData[sceneIndex,2] = data[point + sceneIndex * 4 + 2] == Convert.ToByte(0xFF) ? 1 : data[point + sceneIndex * 4 + 2];
                    light.ScrData[sceneIndex,3] = data[point + sceneIndex * 4 + 3] == Convert.ToByte(0xFF) ? 1 : data[point + sceneIndex * 4 + 3];
                }
                return light;
            }
            catch (Exception ex)
