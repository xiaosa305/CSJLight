...
{
    public partial class MultiplexForm : UIForm
    {
		...
		public MultiplexForm(MainFormBase mainForm, 
			IList<LightAst> lightAstList,
			int stepCount, 			
			IList<int> selectedIndices)
		{
			...
			{
				if ( mainForm.IsSyncMode || selectedIndices.Contains(lightIndex))
				{
		...
		/// <summary>
		/// 事件：《复用次数下拉框》值更改(当值为0时，复用无意义)
		/// </summary>
		/// <param name="sender"></param>
		/// <param name="e"></param>
		private void timesNumericUpDown_ValueChanged(object sender, EventArgs e)
		{
			enterButton.Enabled = timesNumericUpDown.Value != 0;
		}
		...
		{
			if (timesNumericUpDown.Value == 0) {
				MessageBox.Show("复用次数为0，没有意义。");
				return;
			}
