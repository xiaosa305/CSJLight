...
{
    public partial class LibEditForm : UIForm
    {
		...
		private UITextBox[] tdTextBoxes = new UITextBox[32];
		...
		// 原WaySetForm辅助变量
		private UITextBox selectedTextBox = null; //辅助变量，用来记录鼠标选择的textBox
		private int selectedTdIndex = -1;

		public LibEditForm(MainFormBase mainForm)
		{
			InitializeComponent();
			...
			// 动态添加通道预选名称
			IList<string> tdNameList = TextHelper.Read(Application.StartupPath + @"\Default\PreTDNameList");
			...
			for (int tdIndex = 0; tdIndex < 32; tdIndex++)
			{
				...
				tdTextBoxes[tdIndex] = new UITextBox
				{
					Name = "tdTextBoxes" + (tdIndex + 1),
					Cursor = tdTextBoxDemo.Cursor,
					FillColor = tdTextBoxDemo.FillColor ,
					Font = tdTextBoxDemo.Font ,
					Location = tdTextBoxDemo.Location ,
					Margin = tdTextBoxDemo.Margin ,
					Size = tdTextBoxDemo.Size ,
					Style = tdTextBoxDemo.Style,
					TextAlignment = tdTextBoxDemo.TextAlignment 
				};
				...
				tdTextBoxes[tdIndex].Click += tdTextBoxes_Click;
		...
		private void LibEditForm_Load(object sender, EventArgs e)
		{
		...
		{
			playPanel.Visible = tongdaoList != null && tongdaoList.Count > 0;
		}
		...
		private void showTds()
		{
			...
			{
				tdTextBoxes[tdIndex].Text = tongdaoList[tdIndex].TongdaoCommon.TongdaoName;
				tdLabels[tdIndex].Text = LanguageHelper.TranslateWord("通道") + (tdIndex + 1);
				tdTrackBars[tdIndex].Value = tongdaoList[tdIndex].ScrollValue;
				tdNUDs[tdIndex].Value = tongdaoList[tdIndex].ScrollValue;

				string tdRemark = tongdaoList[tdIndex].TongdaoCommon.TongdaoName;
		...
		private void saveLight()
		{
			...
			using (StreamWriter iniWriter = new StreamWriter(fileName))
			{
				...
				for (int tdIndex = 0; tdIndex < tongdaoCount; tdIndex++)
				{
					...
					iniWriter.WriteLine(index + "B=" + tongdaoList[tdIndex].TongdaoCommon.InitValue);
					iniWriter.WriteLine(index + "C=" + tongdaoList[tdIndex].TongdaoCommon.Address);
				}
		...
		private void setUnifyValue(int unifyValue)
		{
			for (int tdIndex = 0; tdIndex < tongdaoList.Count; tdIndex++)
			{
				...
				tongdaoList[tdIndex].ScrollValue = unifyValue;
        ...
        private void setCurrentToInitButton_Click(object sender, EventArgs e)
		{
			...
			if (dr == DialogResult.OK)
			{
				for (int tdIndex = 0; tdIndex < tongdaoList.Count; tdIndex++)
				{
					tongdaoList[tdIndex].TongdaoCommon.InitValue = tongdaoList[tdIndex].ScrollValue;
				}
			}
		}
			...
			if (mainForm.IsEnableOneStepPlay())
			{
				...
				if (tongdaoList != null && tongdaoList.Count > 0)
				{
					foreach (TongdaoWrapper td in tongdaoList)
					{
						...
						// --> tongdaoIndex  = 1 + 1 -2；
						int tongdaoIndex = firstTDValue + td.TongdaoCommon.Address - 2;
		...
		{
			// 设tongdaoWrapper的值			
			tongdaoList[tongdaoIndex].ScrollValue = tdValue;
		...
		private void tdTextBoxSelected()
		{
			...
			tdNumLabel.Text = LanguageHelper.TranslateSentence("已选中: ") + 
				tdLabels[selectedTdIndex].Text + "(" + (selectedTdIndex + 1) + ")" + " - " + tdTextBoxes[selectedTdIndex].Text;
		...
		/// <param name="sender"></param>
		/// <param name="e"></param>
		private void tdTextBoxes_Click(object sender, EventArgs e)
		{
			selectedTextBox = sender as UITextBox;
			tdTextBoxSelected();
		}
		...
		private void tdTextBoxes_LostFocus(object sender, EventArgs e)
		{
			...
			tongdaoList[tdIndex].TongdaoCommon.TongdaoName = tb.Text.Trim(); //更改为最新的名称					
		}
		...
		private void nameListBox_DoubleClick(object sender, EventArgs e)
		{
			if (selectedTextBox != null)
			{
				...
				if (selectedTdIndex != -1)
				{
					tongdaoList[selectedTdIndex].TongdaoCommon.TongdaoName = selectedTextBox.Text.Trim();
				}
			}
			else
			{
		...
		/// <summary>
		/// 辅助方法：清空td相关选项(未选中时，taNamePanel和saPanel都隐藏掉)
		/// </summary>
		private void clearTdRelated(bool clear)
		{
			...
			saAddButton.Visible = !clear;
			saClearButton.Visible = !clear;
			saLabel.Visible = !clear;
		...
		private Panel generateSaPanel(bool newSa, int tdIndex, int saIndex, int startEndValue)
		{
			...
			UITextBox saTextBoxTemp = new UITextBox
			{			
				Cursor = saTextBoxDemo.Cursor,
				FillColor = saTextBoxDemo.FillColor,
				Font = saTextBoxDemo.Font,
				Location = saTextBoxDemo.Location,
				Margin = saTextBoxDemo.Margin,
				Size = saTextBoxDemo.Size,
				Style = saTextBoxDemo.Style,
				TextAlignment = saTextBoxDemo.TextAlignment,
			...
			UIButton saDelButtonTemp = new UIButton
			{
				Location = saDelButtonDemo.Location,
				Size = saDelButtonDemo.Size,
				Font = saDelButtonDemo.Font,
				Cursor = saDelButtonDemo.Cursor ,
				FillColor = saDelButtonDemo.FillColor ,
				FillDisableColor = saDelButtonDemo.FillDisableColor ,
				FillHoverColor = saDelButtonDemo.FillHoverColor ,
				FillPressColor = saDelButtonDemo.FillPressColor ,
				FillSelectedColor = saDelButtonDemo.FillSelectedColor ,
				ForeDisableColor = saDelButtonDemo.ForeDisableColor ,
				RectColor = saDelButtonDemo.RectColor ,
				RectDisableColor = saDelButtonDemo.RectDisableColor ,
				RectHoverColor = saDelButtonDemo.RectHoverColor ,
				RectPressColor = saDelButtonDemo.RectPressColor ,
				RectSelectedColor = saDelButtonDemo.RectSelectedColor ,
				Style = saDelButtonDemo.Style ,	
				Text = saDelButtonDemo.Text,				
			};

			saTextBoxTemp.LostFocus += saTextBox_LostFocus;
			saTextBoxTemp.MouseDoubleClick += saTextBoxes_MouseDoubleClick; // 双击则把通道值设为本子属性的值
		...
		{
			int saIndex = saDict[selectedTdIndex].Controls.IndexOf((sender as Control).Parent);
